...
| --- | --- | --- |
| Detection generalization | Frozen binary models in both dataset directions; three planned training seeds | Full training matrix still running; capped training samples |
| Explanation transfer (RQ3) | Seed-42 LIME, 20 balanced unique feature groups/domain, three explainer seeds, random masking controls, full rankings saved | Pilot cohort; no causal or analyst validation; all model jobs must finish |
...
| --- | --- | --- |
| Source-validation calibration and target-domain calibration drift | Pending implementation | Fit temperature only on source validation; evaluate Brier/ECE without target-test tuning |
| Training-only imbalance strategies | Pending implementation | Compare weighting/resampling without changing test prevalence or leaking synthetic points across splits |
| Alternative explanation baselines and signed evidence | Pending | Test mean/median and supporting/opposing features; do not force desired correlations |
